...
- 파일 손상, 예상과 다른 형식, 쓰기 중 오류 → 감지해서 안내 메시지 출력 후 기본 퀴즈 데이터로 초기화
- 단, 이 초기화는 "이전 상태 복구"가 아니라 "처음 상태로 리셋"이라, 그동안 추가한 퀴즈·기록은 복구되지 않음
    - 개선한다면: 저장할 때마다 백업 파일을 같이 남기고, 손상 시 그 백업으로 먼저 복구를 시도하는 방식 고려 가능
